...
class BaselineDetector:
    ...
    def predict(self, X: np.ndarray) -> pd.DataFrame:
        ...
        if_scores  = self.if_model.decision_function(X_scaled)   # lower = anomalous
        if_labels  = self.if_model.predict(X_scaled)              # -1 or 1

        # LOF in non-novelty mode: use negative_outlier_factor_
        lof_scores = self.lof_model.negative_outlier_factor_      # already computed in fit
        lof_labels = self.lof_model.fit_predict(X_scaled)        # re-run (no novelty)

        # Normalize both to [0,1] where 1 = most anomalous
        def norm_invert(arr):
            a = -arr   # flip so higher = more anomalous
            return (a - a.min()) / (a.max() - a.min() + 1e-9)

        if_norm  = norm_invert(if_scores)
        lof_norm = norm_invert(lof_scores)

        # Ensemble: weighted average (IF slightly more weight)
        ensemble = 0.6 * if_norm + 0.4 * lof_norm

        # Per-user top features
